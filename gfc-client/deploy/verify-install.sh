...
if gfc_need_proxy_dataplane; then
  ...
  if [[ "${GFC_PROXY_MODE:-gateway}" != "bypass" ]]; then
    ...
    check "policy scheme" "grep -qE 'Scheme B|Scheme C|kernel-split|byst-redirect|chain classify|chain classify_non_tcp' /etc/gfc-client/nftables-policy.conf"
    ...
    check "nft mosdns policy" "grep -q 'meta skuid 65353' /etc/gfc-client/nftables-policy.conf"
  ...
  echo "INFO router-only idle — proxy/routing checks skipped"
  check "no policy ip rule 2022" "! ip -4 rule list | grep -qE 'lookup 2022'"
  check "no mangle table loaded" "! nft list table inet gfc_client_mangle 2>/dev/null"
  check "dnsmasq active" "systemctl is-active --quiet dnsmasq"
  if command -v dig >/dev/null; then
    check "dig baidu.com" "dig @127.0.0.1 baidu.com +time=3 +tries=1 +short | grep -q ."
  fi
fi
...
if [[ -f /etc/gfc-client/activation.b32 ]]; then
  ...
  echo "INFO no activation yet (router-only idle OK)"
fi
