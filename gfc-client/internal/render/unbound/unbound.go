...
func (r *Renderer) writeBypassACL() error {
	...
	body := RenderBypassACL(bypassACLActive(r.cfg), proxymode.LoadHosts(r.cfg))
...
func bypassACLActive(cfg *config.Config) bool {
	...
	if env == proxymode.ModeBypass {
		return true
	}
	if proxymode.CommittedMode(cfg) == proxymode.ModeBypass {
		return true
	}
	return proxymode.NormalizeMode(cfg.ProxyMode) == proxymode.ModeBypass
}
...
func RenderBypassACL(active bool, hosts []string) string {
	...
	b.WriteString("# Bypass customer_hosts ACL. Comments only when not bypass.\n")
	if !active {
		return b.String()
	}
