...
const bypassACLFile = "gfc-bypass-acl.conf"

func (r *Renderer) writeBypassACL() error {
	if r.cfg == nil {
		return nil
	}
	dest := filepath.Join(r.cfg.Paths.UnboundConfD, bypassACLFile)
	if err := os.MkdirAll(filepath.Dir(dest), 0o755); err != nil {
		return err
	}
	body := RenderExtraACL(extraACLHosts(r.cfg))
...
func extraACLHosts(cfg *config.Config) []string {
	if cfg == nil {
		return nil
	}
	...
	case proxymode.ModeTransparent:
		l := transparent.LoadLearned(cfg)
		return l.PublicACLHosts()
	default:
		return nil
	}
}

func bypassACLActive(cfg *config.Config) bool {
	return len(extraACLHosts(cfg)) > 0
}

// RenderBypassACL builds gfc-bypass-acl.conf. Hosts may be public IPs; never 0.0.0.0/0.
func RenderBypassACL(active bool, hosts []string) string {
	if !active {
		hosts = nil
	}
	return RenderExtraACL(hosts)
}

// RenderExtraACL is the include body for bypass customer_hosts or transparent
// public cable hosts. RFC1918 is allowed in server:; never 0.0.0.0/0.
func RenderExtraACL(hosts []string) string {
	var b strings.Builder
	b.WriteString("# Generated by GFC — do not edit\n")
	b.WriteString("# Extra ACL: bypass customer_hosts or transparent learned CE. Never 0.0.0.0/0.\n")
